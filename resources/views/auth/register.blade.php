<x-layouts.main>
    <div style="background-image: url(https://i.imgur.com/yb7j0Qn.png);" class="bg-no-repeat bg-cover bg-center">
    <x-auth-card>
        <x-slot name="logo">
            <a href="/">
                <x-objects.application-logo class="w-20 h-20 fill-current text-gray-500" />
            </a>
        </x-slot>

        <!-- Validation Errors -->
        <x-auth-validation-errors class="mb-4" :errors="$errors" />

        <form method="POST" action="{{ route('register') }}">
            @csrf

            <!-- Name -->
            <div>
                <x-objects.label for="username" :value="__('Username')" />

                <x-objects.input id="username" class="block mt-1 w-full" type="text" name="username" :value="old('username')" required autofocus />
            </div>

            <!-- Email Address -->
            <div class="mt-4">
                <x-objects.label for="email" :value="__('Email')" />

                <x-objects.input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')" required />
            </div>

            <!-- Password -->
            <div class="mt-4">
                <x-objects.label for="password" :value="__('Password')" />

                <x-objects.input id="password" class="block mt-1 w-full"
                                type="password"
                                name="password"
                                required autocomplete="new-password" />
            </div>

            <!-- Confirm Password -->
            <div class="mt-4">
                <x-objects.label for="password_confirmation" :value="__('Confirm Password')" />

                <x-objects.input id="password_confirmation" class="block mt-1 w-full"
                                type="password"
                                name="password_confirmation" required />
            </div>

            <div class="flex items-center justify-end mt-4">
                <a class="underline text-sm text-gray-600 hover:text-gray-900" href="{{ route('login') }}">
                    {{ __('Already registered?') }}
                </a>

                <button class="ml-4">
                    {{ __('Register') }}
                </button>
            </div>
        </form>
    </x-auth-card>
</div>
</x-layouts.main>
